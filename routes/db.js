...
// Oracle Auto Commit 설정 autocommit : 데이터 변경 작업에 대한 SQL 자체가 바로 반영되는 것
...
router.post("/dbTestInsert", function (request, response) {
    oracledb.getConnection({
        ...
        // PrepareStatement 구조 let query =   "INSERT INTO EMP( EMPNO ,ENAME, JOB, MGR,
        // HIREDATE, SAL, COMM, DEPTNO) " +   "VALUES( :EMPNO ,:ENAME, :JOB, :MGR,
        // SYSDATE, :SAL, :COMM, :DEPTNO )"; let binddata = [
        // Number(request.body.empno),   request.body.ename,   request.body.job,
        // request.body.mgr,   Number(request.body.sal),   Number(request.body.comm),
        // Number(request.body.deptno), ]; PrepareStatement 구조
        let query = "INSERT INTO LOCAL(LOCALNO, LOCALNAME) VALUES( :LOCALNO, :LOCALNAME)";
