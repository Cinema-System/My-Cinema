...
CREATE TABLE MOVIERATE(
    ...
    FOREIGN KEY(MVNO) REFERENCES LOCAL(MVNO)
    PRIMARY KEY(MVNO)
);
...
CREATE TABLE SEAT (
    SEATNO NUMBER,    -- 좌석번호
    PRIMARY KEY(SEATNO),
    FOREIGN KEY(SCHNO) REFERENCES SCHEDULE(SCHNO)   -- 상영일정_식별번호
);
...
CREATE TABLE BOOKSEAT(  
    BOOKSEATNO NUMBER;                                  --자리정보_식별번호
    FOREIGN KEY(BOOKINGNO) REFERENCES BOOKING_INFO(BOOKINGNO),-- 예매정보_식별번호
...
CREATE TABLE USER(
    ...
    MEMBERCHECK NUMBER,    -- 회원체크 (0=비회원, 1=회원, 2=관리자)
    PRIMARY KEY(USERID)
);
...
CREATE TABLE MANAGER(
    MNGPWD VARCHAR2,        -- 비밀번호 
    MNGNAME VARCHAR2,       -- 이름
    FOREIGN KEY(MNGID) REFERENCES USER(USERID), -- 아이디
    PRIMARY KEY(MNGID)
);
...
CREATE TABLE POINT_INFO_LIST(
    POINTTIME NUMBER,                                               -- 활용시각
    FOREIGN KEY(POINTNO) REFERENCES POINT_INFO(POINTNO),            -- 포인트정보_식별번호
    PRIMARY KEY(POINTNO),
    PRIMARY KEY(POINTTIME)
);
